...
### QC STEP 2: SEX DISCREPANCY CHECK ###
echo "=== QC STEP 2: SEX DISCREPANCY CHECK ==="
...
# Count sex discrepancies
sex_discord=$(awk 'NR > 1 && $5 == "PROBLEM"' breast_eur_mind.sexcheck | wc -l)
echo "Sex discrepancies found: $sex_discord"

# Remove sex discrepant individuals
if [[ $sex_discord -gt 0 ]]; then
    awk 'NR > 1 && $5 == "PROBLEM" {print $1, $2}' breast_eur_mind.sexcheck > fail-sex-discord.txt
    plink --bfile breast_eur_mind \
          --remove fail-sex-discord.txt \
          --make-bed \
          --threads 16 \
          --out breast_eur_sex
else
    cp breast_eur_mind.bed breast_eur_sex.bed
    cp breast_eur_mind.bim breast_eur_sex.bim
    cp breast_eur_mind.fam breast_eur_sex.fam
    touch fail-sex-discord.txt
fi

track_counts "sex_filter" "breast_eur_sex"

### QC STEP 3: MAF CHECK (DOUBLE CHECK) ###
echo "=== QC STEP 3: MAF CHECK ==="

# Check current MAF distribution
plink --bfile breast_eur_sex \
      --freq \
      --threads 16 \
      --out breast_eur_sex

low_maf=$(awk 'NR > 1 && $5 < 0.05' breast_eur_sex.frq | wc -l)
...
if [[ $low_maf -gt 0 ]]; then
    ...
    cp breast_eur_sex.bed breast_eur_maf.bed
    cp breast_eur_sex.bim breast_eur_maf.bim
    cp breast_eur_sex.fam breast_eur_maf.fam
fi

track_counts "maf_filter" "breast_eur_maf"

### QC STEP 4: HWE CHECK ###
echo "=== QC STEP 4: HARDY-WEINBERG EQUILIBRIUM CHECK ==="
...
echo "=== QC STEP 5: HETEROZYGOSITY CHECK ==="
...
### QC STEP 6: IBD/RELATEDNESS ###
echo "=== QC STEP 6: IBD/RELATEDNESS CHECK ==="
...
### QC STEP 7: DIFFERENTIAL MISSINGNESS ###
echo "=== QC STEP 7: DIFFERENTIAL MISSINGNESS ==="
...
samples_removed_sex=$((${counts[mind_filter_samples]} - ${counts[sex_filter_samples]}))
variants_removed_maf=$((${variants[sex_filter_variants]} - ${variants[maf_filter_variants]}))
...
if [[ ! -s "$QC_LOG" ]] || ! grep -q "^Dataset" "$QC_LOG"; then
    echo -e "Dataset\tDate\tInitial_Samples\tInitial_Variants\tEUR_Samples\tEUR_Removed\tGeno_Variants_Removed\tMind_Samples_Removed\tSex_Samples_Removed\tMAF_Variants_Removed\tHWE_Variants_Removed\tHet_Samples_Removed\tIBD_Samples_Removed\tDiffMiss_Variants_Removed\tFinal_Samples\tFinal_Variants\tFinal_Cases\tFinal_Controls" > "$QC_LOG"
fi

# Append summary row
echo -e "breast_eur\t$(date '+%Y-%m-%d %H:%M:%S')\t${counts[initial_samples]}\t${variants[initial_variants]}\t${counts[eur_ancestry_samples]}\t$samples_removed_ancestry\t$variants_removed_geno\t$samples_removed_mind\t$samples_removed_sex\t$variants_removed_maf\t$variants_removed_hwe\t$samples_removed_het\t$samples_removed_ibd\t$variants_removed_diffmiss\t${counts[final_samples]}\t${variants[final_variants]}\t$final_cases\t$final_controls" >> "$QC_LOG"

### CLEANUP INTERMEDIATE FILES ###
echo "=== CLEANING UP INTERMEDIATE FILES ==="

# Remove intermediate PLINK files but keep QC failure lists and final files
rm -f breast_eur_raw.* breast_eur_phenotypes.* breast_eur_geno.* breast_eur_mind.* breast_eur_sex.* breast_eur_maf.* breast_eur_hwe.* breast_eur_het.* breast_eur_ibd.*
rm -f common_snps_raw.* common_snps.* all_ids_sex.txt
rm -f *.log *.nosex *.frq *.hwe *.het *.genome *.sexcheck *.missing
